    ...
    <view class="form-content">
      <view class="form-section">
        <view class="section-header">
          <text class="section-title">销售单信息</text>
        ...
        <view v-if="infoExpanded" class="field-list">
          ...
          <view class="field-row">
            ...
            <view v-else class="picker-value">
              <text>{{ selectedCashierName || '-' }}</text>
            </view>
          </view>
          <view class="field-row">
            <text class="field-label">销售类型</text>
            <picker
      ...
      <view class="form-section">
        <view class="field-row required-row">
          ...
          <customer-autocomplete
            ...
            @select="handleCustomerTextSelected"
          />
        </view>
          ...
          >
            <view class="goods-item-header">
              ...
              <view class="goods-item-main">
                ...
                <text class="goods-item-meta">
                  {{ item.unit || '-' }} · 库存 {{ formatQuantity(item.stock) }}
                </text>
              </view>
      ...
      <view v-if="form.items.length">
        <view class="form-section">
          ...
          <view class="field-row">
            <text class="field-label">本次收款</text>
            <view class="inline-input">
              <input
                v-model="form.paidAmount"
                class="text-input"
                type="digit"
                @blur="normalizeMoneyFields"
              />
              <text class="paid-state" @click="fillPaidAmount">已收</text>
            </view>
          </view>
          <view class="field-row note-row">
           ...
           <view class="field-row">
            ...
            >
              <view class="picker-value">
                <text :class="{ placeholder: !form.settleId }">
                  {{ selectedSettleName || '请选择结算账户' }}
                </text>
                <uni-icons type="right" color="#999" :size="18" />
              </view>
            </picker>
        ...
        <view class="summary-section">
          ...
          <view class="summary-row secondary">
            <text>折后金额</text>
            <text>{{ formatAmount(form.discountedAmount) }}</text>
          </view>
          <view class="summary-row secondary">
            <text>应收金额</text>
            <text>{{ formatAmount(form.payableAmount) }}</text>
          </view>
          <view class="summary-row secondary">
            <text>未收金额</text>
            <text class="unpaid-amount">{{ formatAmount(form.unpaidAmount) }}</text>
          </view>
        </view>
      ...
      <view class="form-actions" style="margin-top: 22rpx;">
        <button
          v-if="isEditMode"
          class="delete-button"
          :disabled="saving"
...
export default {
  ...
  computed: {
    ...
    selectedSettleName() {
      const option = this.settleOptions.find(
        (item) => String(item.value) === String(this.form.settleId)
      )
  ...
  methods: {
    async initialize() {
      ...
      try {
        ...
        uni.showToast({
          title: error && error.message ? error.message : '销售单初始化失败',
          icon: 'none'
        })
      } finally {
    ...
    async loadSettles() {
      ...
      this.settleOptions = (Array.isArray(data) ? data : [])
        ...
        .map((item) => ({
          label: item.name || `账户${item.id}`,
          value: String(item.id)
        }))
      if (!this.form.settleId && this.settleOptions.length) {
        this.form.settleId = this.settleOptions[0].value
      }
    ...
    applyOrder(order) {
      ...
      form.customerId = String(order.customerId || '')
      form.customerName = order.customerId_dictText || order.customerName || ''
      form.cashierId = String(order.cashierId || this.user.id || '')
      form.cashierName = order.cashierName || ''
    ...
    async generateOrderNo() {
      ...
      this.form.orderNo = await createSaleOrderNo()
    },
    handleCustomerSelected(payload) {
      if (!payload) return
      this.form.customerId = String(payload.id || '')
      this.form.customerName = String(payload.name || '')
      if (
        payload.raw &&
        payload.raw.discount !== undefined &&
        payload.raw.discount !== null &&
        payload.raw.discount !== ''
      ) {
        this.form.discountRate = toNumber(payload.raw.discount, 100)
        this.recalculate()
      }
    },
    handleCustomerTextSelected(value) {
      if (!value) {
    ...
    onDateChange(event) {
      ...
      if (!value) return
      this.form.createTime = new Date(`${value}T00:00:00`)
    },
    ...
    openGoodsSelect() {
      const storageKey = `easy-store-sale-selection-${Date.now()}`
      uni.setStorageSync(storageKey, this.form.items)
      uni.navigateTo({
        url: `/pages/goods-select/index?storageKey=${encodeURIComponent(storageKey)}`,
        success: (res) => {
          const channel = res && res.eventChannel
          if (!channel) return
          channel.on('selected', (items) => {
            this.mergeSelectedItems(items)
          })
        }
      })
    },
    ...
    handleItemChange(item) {
      const quantity = Math.max(0, roundQuantity(item.displayQuantity, 0))
    ...
    recalculate() {
      ...
      this.form.items.forEach((item) => {
        const quantity = Math.max(
          0,
          roundQuantity(item.displayQuantity !== undefined ? item.displayQuantity : item.quantity)
        )
        const unitPrice = toNumber(item.unitPrice)
        item.quantity = quantity
        item.totalAmount = this.roundMoney(quantity * unitPrice * sign)
      ...
      this.form.payableAmount = this.roundMoney(
        this.form.discountedAmount + toNumber(this.form.freightAmount)
      )
      this.form.paidAmount = this.roundMoney(toNumber(this.form.paidAmount))
      this.form.unpaidAmount = this.roundMoney(
        this.form.payableAmount - this.form.paidAmount
      )
    },
    handleDiscountRateChange() {
      this.form.discountRate = Math.min(
        100,
        Math.max(0, toNumber(this.form.discountRate, 100))
      )
      this.recalculate()
    },
    ...
    fillPaidAmount() {
      this.form.paidAmount = this.roundMoney(toNumber(this.form.payableAmount))
      this.recalculate()
    },
    buildPayload() {
      this.recalculate()
      return {
        ...
        items: this.form.items.map((item) => ({
          ...
          quantity: this.form.orderType === 2
            ...
            : Math.abs(roundQuantity(item.displayQuantity)),
          unitPrice: toNumber(item.unitPrice),
          totalAmount: this.form.orderType === 2
            ? -Math.abs(toNumber(item.totalAmount))
            : Math.abs(toNumber(item.totalAmount)),
          note: item.note || ''
        }))
      }
    ...
    async saveForm() {
      ...
      try {
        ...
        if (this.form.id) await editSaleOrder(payload)
        else await addSaleOrder(payload)
    ...
    async deleteForm() {
      ...
      try {
        await removeSaleOrder({
          id: this.form.id,
          userId: this.user.id
        })
        await this.resetForm(false)
        uni.showToast({ title: '删除成功', icon: 'success' })
        setTimeout(() => uni.navigateTo({ url: '/pages/sale-order/list' }), 350)
      } catch (error) {
        uni.showToast({
    ...
    syncStatusIndex() {
      const index = this.statusOptions.findIndex(
        (item) => Number(item.value) === Number(this.form.status)
      )
      this.statusIndex = index >= 0 ? index : 1
    },
    syncSettleIndex() {
      const index = this.settleOptions.findIndex(
        (item) => String(item.value) === String(this.form.settleId)
      )
    ...
    openHistory() {
      uni.navigateTo({ url: '/pages/sale-order/list' })
    },
    redirectToLogin() {
...
.paid-state.paid { color: #00a870; border-color: #55c99b; }
