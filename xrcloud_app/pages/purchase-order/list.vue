    ...
    <uni-popup ref="filterPopup" type="bottom" :safe-area="true" :is-mask-click="true">
      <view class="filter-popup">
        ...
        <view class="popup-form-item switch-item">
          <text class="popup-label">仅看未付</text>
          <switch :checked="filters.unpaidOnly" color="#722ed1" @change="onUnpaidOnlyChange" />
        </view>
      ...
      <view v-for="order in records" :key="order.id" class="order-card">
        <view class="card-header">
          <view class="order-main">
            ...
            <text class="order-type" :class="{ return: Number(order.orderType) === 2 }">
              {{ Number(order.orderType) === 2 ? '退货' : '进货' }}
            </text>
          </view>
        ...
        <view class="supplier-row">
          <view class="supplier-name">
            <uni-icons type="shop" color="#722ed1" :size="18" />
            <text>{{ order.supplierId_dictText || order.supplierName || order.supplierId || '未设置供应商' }}</text>
          </view>
        ...
        <view class="amount-grid">
          ...
          <view class="amount-item">
            ...
            <text class="amount-value">{{ formatAmount(order.paidAmount) }}</text>
          </view>
          <view class="amount-item">
            <text class="amount-label">未付</text>
            <text class="amount-value debt">{{ formatAmount(order.unpaidAmount) }}</text>
          </view>
          <view v-if="Number(order.freightAmount || 0) !== 0" class="amount-item freight-item">
            <text class="amount-label">运费</text>
            <text class="amount-value">{{ formatAmount(order.freightAmount) }}</text>
          </view>
        </view>
...
import { getUserInfo, listPurchaseOrders, listAppUsers } from '../../common/api'
...
export default {
  ...
  computed: {
    ...
    activeFilterCount() {
      ...
      if (this.filters.unpaidOnly) count += 1
      return count
    }
  },
  onShow() {
  ...
  methods: {
    ...
    buildQuery(current) {
      ...
      const supplierId = String(this.filters.supplierId || '').trim()
      ...
      if (this.filters.unpaidOnly) params.unpaidOnly = 1
    ...
    async queryOrders(reset = false, pageNumber, showPageLoading = false) {
      ...
      try {
        const page = await listPurchaseOrders(this.buildQuery(targetPage))
        ...
        const message = error && error.message ? error.message : '进货单加载失败'
    ...
    onUnpaidOnlyChange(event) {
      this.filters.unpaidOnly = Boolean(event.detail.value)
    },
    formatDate(value) {
...
.order-main,
.supplier-name { display: flex; align-items: center; min-width: 0; }
...
.supplier-row { margin-top: 18rpx; color: #6f6979; font-size: 23rpx; }
.supplier-name { flex: 1; gap: 8rpx; overflow: hidden; }
.supplier-name text { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.freight-item { flex-basis: 100%; align-items: flex-end; padding: 12rpx 22rpx 0 0; box-sizing: border-box; border-top: 1rpx solid #eeeaf4; }
.amount-label { color: #918b99; font-size: 20rpx; }
.amount-value { margin-top: 7rpx; color: #34303d; font-size: 23rpx; font-weight: 600; }
.amount-value.debt { color: #e5484d; }
