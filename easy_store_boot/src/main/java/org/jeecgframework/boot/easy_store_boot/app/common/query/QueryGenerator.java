package org.jeecgframework.boot.easy_store_boot.app.common.query;
...
public class QueryGenerator {
	...
	public static <T> QueryWrapper<T> initQueryWrapper(T searchObj,JSONObject param){
		QueryWrapper<T> queryWrapper = new QueryWrapper<T>();
		Map<String,String[]> parameterMap = JsonConvertToMap(param);
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				...
				if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
					...
					final String field = CommonUtils.camelToUnderline(name);
					if(vals.length>1) {
						queryWrapper.and(j -> {
	...
	public static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap) {
		...
		if (StringUtils.isNotEmpty(column) && StringUtils.isNotEmpty(order)) {
			...
			if (order.toUpperCase().indexOf(ORDER_TYPE_ASC)>=0) {
				queryWrapper.orderByAsc(CommonUtils.camelToUnderline(column));
			} else {
				queryWrapper.orderByDesc(CommonUtils.camelToUnderline(column));
			}
		}
	}
	...
	private static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		if (value == null || rule == null || value == null || StringUtils.isEmpty(value.toString())) {
			return;
		}
	...
	public static Map<String, String[]> JsonConvertToMap(JSONObject jsonObject) {
		...
		for (Map.Entry<String, Object> entry : jsonObject.entrySet()) {
			...
			if (value instanceof String[]) {
				...
				parameterMap.put(key, toStringArray(iterable));
			} else if (value != null) {
				// 其他类型转字符串
