...
@RestController
@RequestMapping("api/user/appSaleStatistics")
public class AppSaleStatisticsController {
    ...
    public Result<?> list(@RequestBody JSONObject param) {
        ...
        String pageSql = aggregateSql + " ORDER BY goodsName, goodsId LIMIT ? OFFSET ?";
        ...
        String countSql = "SELECT COUNT(*) FROM (SELECT 1 " + baseSql() + condition.whereSql +
                " GROUP BY i.goods_id, COALESCE(g.title, ''), COALESCE(i.unit, g.unit, '')) t";
        ...
        String totalSql = "SELECT COALESCE(SUM(i.quantity), 0) AS quantityTotal, " +
                "COALESCE(SUM(i.total_amount), 0) AS amountTotal " +
    ...
    public Result<?> detail(@RequestBody JSONObject param) {
        ...
        String sql = "SELECT o.order_no AS orderNo, " + SALE_BUSINESS_TIME_SQL + " AS businessTime, " +
                ...
                "COALESCE(i.total_amount, 0) AS amount, COALESCE(o.discount_rate, 100) AS discountRate, " +
        ...
        for (Map<String, Object> row : jdbcTemplate.queryForList(sql, params.toArray())) {
            ...
            double receivableAmount = amount * discountRate / 100D;
            JSONObject record = new JSONObject();
            record.put("rowNo", rowNo++);
            ...
            record.put("discountAmount", amount - receivableAmount);
    ...
    private String detailBaseSql() {
        return baseSql() +
                "LEFT JOIN app_customer c ON c.id = o.customer_id " +
                "LEFT JOIN app_user u ON u.id = o.cashier_id ";
    }
