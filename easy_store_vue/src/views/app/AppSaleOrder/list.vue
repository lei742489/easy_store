<template>
  <div class="container">
    <a-card class="general-card" title="销售单" style="margin-top: 12px">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
            ...
            <a-row :gutter="24">
              <a-col :span="7">
                <a-form-item field="customerId" label="客户名称">
                  <SearchModal
                    v-model:customer-id="formModel.customerId"
                    placeholder="请选择"
                  ></SearchModal>
                </a-form-item>
              </a-col>
...
<script lang="ts" setup>
  ...
  const generateFormModel = (): AppSaleOrder => {
    return {
      id: undefined,
      orderNo: undefined,
  ...
  const renderData = ref<AppSaleOrder[]>([]);
  ...
  const isPaidAmountMismatch = (record: AppSaleOrder) => {
    const payableAmount = toAmountNumber(record.payableAmount);
    const paidAmount = toAmountNumber(record.paidAmount);
    return Math.abs(payableAmount - paidAmount) >= 0.005;
  };
  ...
  const columns = computed<TableColumnData[]>(() => [
    ...
    },
    {
      ...
      render: (record) => {
        ...
        return getPriceStrByH((data as any).totalAmount);
      },
    },
    {
      title: '应收金额',
    ...
    },
    {
      ...
      render: (record) => {
        ...
        return h(
          ...
          {
            class: {
              'amount-mismatch': isPaidAmountMismatch(data as AppSaleOrder),
            },
          },
          getPriceStrByH((data as any).paidAmount)
        );
      },
    },
    {
      ...
      ? [
          {
            ...
            render: (record) => {
              const data = record.record;
              return getPriceStrByH((data as any).grossProfit);
            },
          } as TableColumnData,
        ]
      : []),
    ...
    },
    {
      ...
      render: (record) => {
        ...
        return time !== undefined ? time.split(' ')[0] : '';
      },
    },
    {
      title: '操作',
  ...
  const handelRemove = async (data: AppSaleOrder) => {
    await remove(data);
    Message.success('操作成功');
    search();
  };

  const canApprove = (record: AppSaleOrder) => {
  ...
  const handleApprove = async (data: AppSaleOrder) => {
    if (!data.id) return;
  ...
  const exportXls = (data: AppSaleOrder) => {
