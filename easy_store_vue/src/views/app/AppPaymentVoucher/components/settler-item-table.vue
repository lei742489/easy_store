...
<script lang="ts" setup>
  ...
  import { listByPaymentId } from '../api/api-AppPaymentSettleItem';

  const { loading, setLoading } = useLoading(false);

  const renderData = ref<AppPaymentSettleItem[]>([]);
  const totalAmount = defineModel<number>('totalAmount');

  const defaultItem: AppPaymentSettleItem = {
    id: undefined,
    settleId: undefined,
  ...
  const initData = (size = 0) => {
    ...
    for (let i = 0; i < max; i += 1) {
      const item: AppPaymentSettleItem = {};
  ...
  const getItemsList = () => {
    const result: AppPaymentSettleItem[] = [];
