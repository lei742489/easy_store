<template>
  <div class="container">
    <a-card class="general-card" title="收款单" style="margin-top: 12px">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
            ...
            <a-row :gutter="24">
              <a-col :span="7">
                <a-form-item field="supplierId" label="客户名称">
                  <SearchModal
                    v-model:supplier-id="formModel.customerId"
                    placeholder="请选择"
                  ></SearchModal>
                </a-form-item>
              </a-col>
...
<script lang="ts" setup>
  ...
  const generateFormModel = (): AppReceivePaymentVoucher => {
    return {
      id: undefined,
      orderNo: undefined,
  ...
  const renderData = ref<AppReceivePaymentVoucher[]>([]);
  ...
  const columns = computed<TableColumnData[]>(() => [
    ...
    },
    {
      ...
      render: (record) => {
        const data = record.record as AppReceivePaymentVoucher;
        return h(
          'span',
  ...
  const handelRemove = async (data: AppReceivePaymentVoucher) => {
    await remove(data);
    Message.success('操作成功');
    search();
  };

  const canApprove = (record: AppReceivePaymentVoucher) => {
  ...
  const handleApprove = async (data: AppReceivePaymentVoucher) => {
    if (!data.id) return;
  ...
  const exportXls = (data: AppReceivePaymentVoucher) => {
