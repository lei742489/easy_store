<template>
  <div class="container">
    <a-card class="general-card" title="进货单" style="margin-top: 12px">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
      ...
      >
        ...
        <template #expand-row="{ record }">
          <item-expand :item-list="record.items"></item-expand>
        </template>
      </a-table>
    </a-card>
...
<script lang="ts" setup>
  ...
  const generateFormModel = (): AppPurchaseOrder => {
    return {
      id: undefined,
      orderNo: undefined,
  ...
  const renderData = ref<AppPurchaseOrder[]>([]);
  ...
  const handelRemove = async (data: AppPurchaseOrder) => {
    await remove(data);
    Message.success('操作成功');
    search();
  };

  const canApprove = (record: AppPurchaseOrder) => {
  ...
  const handleApprove = async (data: AppPurchaseOrder) => {
    if (!data.id) return;
  ...
  const handelEdit = (item: AppPurchaseOrder) => {
  ...
  const exportXls = (data: AppPurchaseOrder) => {
